...
export default class Entries extends Component {
  ...
  static fetchData = ({ dispatch, params: { branch, id, owner }, host='' }) => {
    d('fetchData')
    dispatch(RepositoryActions.fetching())
    return Promise
      .all([
        axios.get(`${host}/api/v1/repos/${owner}/${id}/branches`),
        axios.get(`${host}/api/v1/repos/${owner}/${id}/commits`),
        axios.get(`${host}/api/v1/repos/${owner}/${id}/tags`),
        axios.get(`${host}/api/v1/repos/${owner}/${id}/git/trees/${branch ? branch : ''}?last_commit=1`)
      ])
      .then(([ { data: branches }, { data: commits }, { data: tags }, { data: { tree: entries } } ]) => {
        dispatch(RepositoryActions.fetchBranchesSuccess(branches))
        dispatch(RepositoryActions.fetchCommitsSuccess(commits))
        dispatch(RepositoryActions.fetchTagsSuccess(tags))
  ...
  render = () => {
    d(`render (entries[${this.props.entries.length}])`)
                    ...
                    return (
                      <tr key={path}>
                        ...
                        <td className={styles.name}>
                          <Link to={`/${owner}/${id}/${type}/${branch}/${basePath ? basePath + '/' : ''}${path}`}>
                            {path}
                          </Link>
                        </td>
