...
export default class FileContent extends Component {
  ...
  static fetchData = ({ dispatch, branch, path, repo, user }) => {
    dispatch(RepositoryActions.fetching())
    return axios.get(`http://localhost:3000/api/v1/users/${user}/repositories/${repo}/branches/${branch}/entries/${path}`)
  ...
  render = () => {
    const { params: { branch, path, repo, user}, entry: { bytes, content, lines } } = this.props
    return (
            ...
            <div className={styles.fileActions}>
              <div className={btnStyles.group}>
                <Link
                  ...
                  to={`/${user}/${repo}/raw/${branch}/${path}`}
                >
                  Raw
                </Link>
                <Link
                  className={btnStyles.defaultSmBtn}
                  to={`/${user}/${repo}/blame/${branch}/${path}`}
                >
                  Blame
                </Link>
                <Link
                  className={btnStyles.defaultSmBtn}
                  to={`/${user}/${repo}/commits/${branch}/${path}`}
                >
                  History
                </Link>
              </div>
