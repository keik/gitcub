...
export default class FileContent extends Component {
  ...
  componentWillMount() {
    ...
    const { dispatch, fetched, params: { branch, owner, id, splat: path='' } } = this.props
    /* if (!fetched) {*/
    FileContent.fetchData({
      dispatch,
      uri: `http://localhost:3000/api/v1/users/${owner}/repositories/${id}/branches/${branch}/entries/${path}`,
      path,
    })
  }
