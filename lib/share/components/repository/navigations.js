...
export default class RepoNavigations extends React.Component<{
  issuesCount: number,
  params: {
    owner: string,
    repo: string
      ...
      <div className={styles.container}>
        <nav className={styles.reponav}>
          <ul onClick={e => e.preventDefault()}>
            ...
            <li
              className={/^issues/.test(lastRoutePath) ? styles.selected : ''}
            >
            ...
            <li className={/^pulls/.test(lastRoutePath) ? styles.selected : ''}>
              <Link to={`/${owner}/${repo}/pulls`}>
                <i className="fa fa-code-fork" />Pull requests{' '}
                {pullRequestsCount}
              </Link>
            </li>
            <li
            ...
            {session != null ? (
              <li
                className={/^wiki/.test(lastRoutePath) ? styles.selected : ''}
              >
            ...
            <li
              className={/^graphs/.test(lastRoutePath) ? styles.selected : ''}
            >
            ...
            {session != null ? (
              <li
                className={
                  /^settings/.test(lastRoutePath) ? styles.selected : ''
                }
              >
