              ...
              fetchOpts: [
                {
                  ...
                  uri: ({ id, owner }) => `${host}/api/v1/repos/${owner}/${id}/branches`,
                },
                {
                  isRequired: (state) => (!state.commits),
                  merge: res => ({commits: res.data}),
                  uri: ({ id, owner }) => `${host}/api/v1/repos/${owner}/${id}/commits`,
                },
                {
                  isRequired: (state) => (!state.commits),
                  merge: res => ({entries: res.data.tree}),
                  uri: ({ branch, id, owner }) => `${host}/api/v1/repos/${owner}/${id}/git/trees/${branch ? branch : ''}?last_commit=1`,
                },
                {
                  isRequired: (state) => (!state.commits),
                  merge: res => ({tags: res.data}),
                  uri: ({ id, owner }) => `${host}/api/v1/repos/${owner}/${id}/tags`,
                },
              ],
              store,
            }
        ...
        getComponent={connectWithInitialFetch(
            ...
            store,
            {
              fetchOpts: {
                ...
                uri: ({ id, owner, branch, splat: path }) => `${host}/api/v1/repos/${owner}/${id}/contents/${path}?ref=${branch}`,
              },
              store,
            }
