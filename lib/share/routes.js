...
export default (store) => (
  <Route path="/" component={connect(state => state)(App)}>
    ...
    />
    <Route path="/new"
      component={connect(state => state)(New)} />
    <Route path="/:owner"
            ...
            state => state.repository,
            {
              fetchOpts: [
                ...
                },
                {
                  isRequired: (state) => (!state.commits),
                  merge: data => ({commits: data || []}),
                  uri: ({ repo, owner, tree }) => `${ROOT_URL}/api/v1/repos/${owner}/${repo}/commits?sha=${tree}`,
                },
                {
                  isRequired: (state) => (!state.entries || state.entries.length === 1),
            ...
            state => state.repository,
            {
              fetchOpts: [
                ...
                },
                {
                  isRequired: (state) => (!state.commits),
                  merge: data => ({commits: data || []}),
                  uri: ({ repo, owner, tree, splat: path }) => `${ROOT_URL}/api/v1/repos/${owner}/${repo}/commits?sha=${tree}`,
                },
                {
