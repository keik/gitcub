  ...
  <Route path="/" component={connect(state => state)(App)}>
    <IndexRoute
      getComponent={connectWithInitialFetch(state => state.repository, {
        fetchOpts: [
          {
            isRequired: state => !state.repositories,
            merge: data => ({ repositories: data || [] }),
            uri: () => `${ROOT_URL}/api/v1/repositories`
          }
        ],
        store
      })(Home)}
    />
    <Route path="/new" component={connect(state => state)(New)} />
    <Route path="/:owner" component={connect(state => state)(User)} />
    <Route
      path="/:owner/:repo"
    ...
    >
        ...
        getComponent={connectWithInitialFetch(state => state.repository, {
          fetchOpts: [
            ...
            },
            {
              ...
              uri: ({ repo, owner, tree, splat: path }) =>
      ...
      <Route
        ...
        getComponent={connectWithInitialFetch(
          ({ repository }, { params: { splat: path } }) =>
            repository.entries.find(e => e.path === path),
          {
            fetchOpts: {
              isRequired: state => !state || !state.content,
              merge: data => ({ entries: [data] }),
              uri: ({ repo, owner, branch, splat: path }) =>
                `${ROOT_URL}/api/v1/repos/${owner}/${repo}/contents/${path}?ref=${branch}`
            },
            store
          }
        )(RepoFileContent)}
