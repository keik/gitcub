...
test.cb(`POST ${API_REPOS.LOGIN_USERS} should return 201 when specified name repository does not exist`, (t) => {
  axios.post(`http://localhost:${PORT}${API_REPOS.LOGIN_USERS}`, { name: 'repo2', __owner: 'DUMMY' })
    .then((res) => {
      t.is(res.status, 201)
    ...
    .finally(() => {
      rimraf.sync(path.join(REPO_ROOT, 'DUMMY', 'repo2'))
      t.end()
    })
})

test.cb(`POST ${API_REPOS.LOGIN_USERS} should return 409 when specified name repository already exists`, (t) => {
  axios.post(`http://localhost:${PORT}${API_REPOS.LOGIN_USERS}`, { name: 'repo1', __owner: 'user1' })
    .then((res) => {
      t.fail(res.toString())
    })
    .catch((err) => {
