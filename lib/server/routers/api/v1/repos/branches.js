...
export default new Router().get(API_REPOS_BRANCHES, (onGet: any))

function onGet(req, res) {
  const { owner, repo } = req.params
  Git.Repository.openBare(join(REPO_ROOT, owner, repo))
    .then(grepo =>
      grepo.getReferences(Git.Reference.TYPE.LISTALL).then(refs =>
        Promise.all(
          refs.filter(ref => ref.isBranch()).map(ref =>
            grepo.getReferenceCommit(ref).then(commit => ({
              name: ref.shorthand(),
