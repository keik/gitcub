...
export default new Router()
  ...
  .get('/api/v1/repos/:owner/:repo/git/trees(/:sha)?', (req, res) => {
    const config = req.app.get('config')
    const { url } = req
  ...
  .get('/api/v1/users/:user/repositories/:repo/branches/:branch/entries/*', (req, res) => {
    const config = req.app.get('config')
    const { branch, repo, user } = req.params
