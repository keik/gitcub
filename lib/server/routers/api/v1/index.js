...
export default new Router()
  ...
  .get('/api/v1/users/:user/repositories/:repo/branches', (req, res) => {
    ...
    RepositoryService.getBranches(config, user, repo).then(branches => {
      res.json(branches)
    }).catch(err => {
      console.error(err)
  ...
  .get('/api/v1/users/:user/repositories/:repo/tags', (req, res) => {
    ...
    const user = req.params.user
    const repo = req.params.repo
    RepositoryService.getTags(config, user, repo).then(tags => {
      res.json(tags)
    }).catch(err => {
      console.error(err)
      res.status(503).end('503')
  ...
  .get('/api/v1/users/:user/repositories/:repo/commits', (req, res) => {
    ...
    const user = req.params.user
    const repo = req.params.repo
    RepositoryService.getCommits(config, user, repo).then(commits => {
      res.json(commits)
    }).catch(err => {
      console.error(err)
      res.status(503).end('503')
