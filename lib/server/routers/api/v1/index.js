...
export default new Router()
  ...
  .get(API_GIT_COMMITS, (req, res) => {
    ...
    Git.Repository.openBare(repoPath)
      ...
      .then(commit => {
        res.json(convertCommitToObject(commit, owner, repo))
      })
      .catch((err) => {
        console.error(err)
        res.status(503).end('503')
              ...
              return new Promise((resolve, reject) => {
                ...
                walker.fileHistoryWalk(path, 10)
                  .then(histories => { // TODO param
                    ...
                    resolve({
                      lastCommit: convertCommitToObject(lastCommit, repo, owner),
                    })
                  })
                  .catch(reject)
              })
...
function convertCommitToObject(commit, owner, repo) {
