...
function onGet(req, res) {
  ...
  function _treeToObject(tree, grepo, gcommit) {
    ...
    if (withLastCommit) {
      return Promise.all(trees.tree.map(entry => {
        const walker = grepo.createRevWalk()
        walker.push(gcommit.sha())
        walker.sorting(Git.Revwalk.SORT.Time)
        function _pickLastCommit(histries) {
          if (histries.length === 0)
            return walker.fileHistoryWalk(entry.path, 100)
              .then(_pickLastCommit)
          return histries[0].commit
        }
        return _pickLastCommit([])
          .then(lastCommit => {
            return merge(entry, {lastCommit: convertCommitToGitCommitObject(lastCommit, owner, repo)})
          })
      })).then(() => {
        return trees
      })
