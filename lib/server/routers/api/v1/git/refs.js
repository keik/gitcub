...
import { API_GIT_COMMITS, API_GIT_REFS, API_GIT_TAGS } from '../../../../../share/constants/api'
...
function onGet(req, res) {
  ...
  function _convertRefToObj(ref) {
    return {
      ref: ref.name(),
      url: genAPIStr(API_GIT_REFS, {owner, repo, '*': ref.name().replace(/^refs\//, '') }),
      object: {
        type: ref.isTag() ? 'tag' : 'commit',
        sha: ref.target().tostrS(),
        url: genAPIStr((ref.isTag() ? API_GIT_TAGS : API_GIT_COMMITS), {owner, repo, sha: ref.target().tostrS()}),
      },
    }
  }
  ...
  Git.Repository.openBare(repoPath)
    .then(repo => {
      return repo.getReferences(Git.Reference.TYPE.LISTALL)
        .then(refs => {
          // perfect match
          const matchRefs = refs.filter(ref => ref.name().replace(/^refs\//, '') === pref)
          ...
          // partial match
          const partialMatchRefs = refs.filter(ref => RegExp(`^${pref}`).test(ref.name().replace(/^refs\//, '')))
