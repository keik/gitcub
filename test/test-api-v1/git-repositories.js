...
export default function(config) {
  test.cb(`POST ${API_GIT_REPOS} should return 201 when specified name repository does not exist`, (t) => {
    axios.post(`http://localhost:${config.PORT}${API_GIT_REPOS}`, { name: 'repo2', __owner: 'DUMMY' })
      .then((res) => {
        t.is(res.status, 201)
  ...
  test.cb(`POST ${API_GIT_REPOS} should return 409 when specified name repository already exists`, (t) => {
    axios.post(`http://localhost:${config.PORT}${API_GIT_REPOS}`, { name: 'repo1', __owner: 'user1' })
      .then((res) => {
        t.fail(res.toString())
      })
      .catch((err) => {
